...
// finishStep sends returnStep signal through socket.io to frontend
eventEmitter.on('stepiFinished', finishStepi);
...
AsyncWatch(procData, 'vcdDump', function(oldValue, newValue){
	if (state.step) {
		socket.emit('returnStep', procData.vcdDump, procData.parsedDisassembly);
	}
	else if (state.stepi) {
...
io.on("connection", function (socket) {
	...
	socket.on('run', function(code) {
		state.run = true;
		if (state.step) procData.proc.stdin.write('continue\n');
